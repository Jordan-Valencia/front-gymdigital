...
export function MembresiasList() {
  ...
  const getEstadoMembresia = (fechaFin: string) => {
    ...
    if (diffDays < 0) {
      return { estado: "Vencida", color: "bg-red-100 text-red-800" };
    } else if (diffDays <= 7) {
      return { estado: "Por vencer", color: "bg-orange-100 text-orange-800" };
    } else {
      return { estado: "Activa", color: "bg-green-100 text-green-800" };
    }
  };
    ...
    <div className="ml-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="relative flex-1 max-w-md group">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border-2 border-gray-100 rounded-xl
              bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {membresiasFiltradas.map((membresia) => {
          ...
          return (
            <div
              key={membresia.id}
              className="group relative bg-white rounded-xl p-6 border border-gray-100 
                overflow-hidden transition-all duration-300
                hover:shadow-[0_0_25px_-5px_rgba(0,0,0,0.1),0_0_10px_-5px_rgba(0,0,0,0.04)]
                hover:border-indigo-200"
            >
              <div
                className="absolute inset-0 bg-gradient-to-br from-indigo-50/50 via-transparent to-transparent 
              ...
              <div className="relative">
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className="absolute top-4 right-4 p-2 text-gray-400 hover:text-indigo-600 
                    ...
                    <div>
                      <h3
                        className="text-lg font-semibold text-gray-900 mb-2 group-hover:text-indigo-600 
                        transition-colors"
                      >
                        {plan?.nombre}
                      </h3>
                ...
                <div className="space-y-4 group-hover:translate-x-1 transition-transform duration-300">
                  <div
                    ...
                    from-gray-50 to-transparent border border-gray-100/50
                    group-hover:border-indigo-100/50 transition-colors"
                  >
                  ...
                  <div
                    className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r 
                    from-emerald-50 to-transparent border border-emerald-100/50
                    group-hover:border-emerald-200/50 transition-colors"
                  >
                ...
                <div className="mt-6 pt-4 border-t border-gray-100">
                  <p
                    className="text-xs text-gray-500 flex items-center gap-2 group-hover:text-indigo-500 
                    transition-colors"
                  >
                    <span className="inline-block w-2 h-2 bg-current rounded-full animate-pulse"></span>
                    Último pago:{" "}
                    {new Date(membresia.fecha_pago).toLocaleDateString("es-ES")}
                  </p>
                </div>
              </div>
            </div>
      ...
      {membresiasFiltradas.length === 0 && (
        <div className="text-center py-16">
          <div
            className="w-24 h-24 bg-gradient-to-br from-gray-100 to-gray-50 rounded-2xl 
            flex items-center justify-center mx-auto mb-6 shadow-inner"
          >
            <CreditCard className="text-gray-400" size={36} />
          </div>
          ...
          <p className="text-gray-500 max-w-sm mx-auto">
            {searchTerm
              ? "No se encontraron membresías con esos términos"
              : "Comienza creando la primera membresía"}
          </p>
        </div>
      )}
    </div>
